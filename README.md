...
python3 -m src.openclaw_sales_pipeline.cli export-operations
...
- `product_analysis_master_schema`: `normalized_product_name`, `qty`, `gross_sales`, `net_sales`, `supply_amount`, `delivery_amount`, `refund_flag`, `business_date` 등을 포함한 품목별 분석 공통 필드다.
- `channel_operating_model`: 안정 채널 / 레거시 / 인증 대기 / 환경 특수 채널 큐와 운영 원칙을 담는다.
...
- Workflow knowledge:
  ...
  - 채널 운영 큐, 인증 우선순위, 레거시 우회 정책, 매출 기준을 함께 포함
- File analysis:
...
- Operations bundle:
 ...
 - Execution policy:
  - `run`은 이제 채널 운영 큐를 반영해서 `stable -> legacy -> auth_wait -> environment_special` 순서로 실행한다.
  - 브라우저 병렬도도 큐별로 다르게 적용한다. `stable`은 기본 병렬도, `legacy`는 최대 2개, `auth_wait`와 `environment_special`은 단독 처리다.
...
```bash
python3 -m src.openclaw_sales_pipeline.cli ingest-downloads \
  --date 2026-04-09 \
...
SMTP 준비 여부는 먼저 이렇게 확인할 수 있다.

```bash
python3 -m src.openclaw_sales_pipeline.cli smtp-check
```

`ingest-downloads`는 기본적으로 파일을 복사한다. 원본 Downloads 정리를 같이 하려면 `--move`를 쓴다.
